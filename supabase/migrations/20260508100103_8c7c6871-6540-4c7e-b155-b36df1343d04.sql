-- Rebuild policies so exposed SECURITY DEFINER helper functions are no longer needed by app users
...
WITH CHECK (
  ...
  AND EXISTS (
    SELECT 1 FROM public.user_roles ur
    WHERE ur.user_id = auth.uid() AND ur.role = 'restaurant_admin'::public.app_role
  )
  AND EXISTS (
    SELECT 1 FROM public.kyc_applications k
    WHERE k.user_id = auth.uid() AND k.status = 'approved'
...
WITH CHECK (
  (
    ...
    AND EXISTS (SELECT 1 FROM public.kyc_applications k WHERE k.user_id = auth.uid() AND k.status = 'approved')
  )
...
USING (
  EXISTS (
    ...
    WHERE o.id = order_items.order_id
      AND (
        (r.owner_id = auth.uid() AND EXISTS (SELECT 1 FROM public.kyc_applications k WHERE k.user_id = auth.uid() AND k.status = 'approved'))
        OR EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role = 'super_admin'::public.app_role)
      )
  )
);
...
-- Remove direct execute permissions that triggered linter warnings
